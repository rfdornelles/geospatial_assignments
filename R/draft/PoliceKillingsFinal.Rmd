...
# 4. Compute original area of each sector
area_tbl <- df_censo2022 |> 
  select(sc_setor) |> 
  mutate(area_orig = st_area(geometry) |> as.numeric()) |> 
  st_drop_geometry()

# 5. Intersect sectors and districts: sectors may cross multiple districts
...
# 6. Aggregate population and income by district
...
# 7. Join population and income back into districts
...
# 1. Load cleaned police death data from prior step
...
The results shown that in the **OLS model** none of the internal predictors were statistically significant. The adjusted R² is negative (−0.024), indicating poor model fit. Most importantly, the residuals from the OLS model exhibit significant spatial autocorrelation (Moran’s I = 0.18, p < 0.01), suggesting that key spatial dependencies are not accounted for. This confirms the need to apply spatial regression techniques.
...
The spatial error model also performs better than OLS, with a significant lambda term (λ = 0.396, p < 0.01) and lower AIC. Like the lag model, it confirms the presence of unobserved spatial processes that affect the outcome. Again, none of the internal variables are individually significant, but the improved residual structure and model diagnostics indicate that spatial models provide a more appropriate framework for capturing the underlying pattern.

Together, these results suggest that district-level victim characteristics and policing force composition alone do not explain the spatial distribution of police violence. However, the spatial structure of the phenomenon is statistically robust. In the following section, we try to include models that account for external structural variables (e.g., income, favela coverage, and urban density) to better understand the social geography of lethal policing.
...
- Population density (pop_density): The number of residents per square kilometer, included to control for urban form and exposure. Denser areas may have different policing dynamics than low-density, peripheral neighborhoods.
...
However, none of the individual district-level predictors — including race, gender, age, or the proportion of incidents involving the Polícia Militar — were statistically significant. Even after incorporating structural variables such as favela coverage, income per capita, and population density, the models failed to identify clear local drivers. Instead, the most consistent finding across all models is that spatial proximity itself matters: high police lethality in one district increases the likelihood of high lethality in its neighbors.
