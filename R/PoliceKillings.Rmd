...
sect_split <- st_intersection(
  df_censo2022 |> select(sc_setor, sc_populac),
...
pop_distritos <- sect_split |> 
  st_drop_geometry() |> 
  group_by(ds_nome) |> 
  summarise(pop_total = sum(pop_share), .groups = "drop")
...
write_sf(df_distritos_pop, here("data", "distritos_pop_2022.geojson"))
```
...
# 2. Calculate area of each district
model_data <- model_data |> 
  mutate(area_total = st_area(geometry))

# 3. Intersect favelas with districts
favela_intersection <- st_intersection(data_favela, model_data)

# 4. Compute favela area within each district using `ds_nome`
favela_area_district <- favela_intersection |> 
  mutate(favela_area = st_area(geometry)) |> 
  group_by(ds_nome) |> 
  summarise(total_favela_area = sum(favela_area)) |> 
  ungroup()

# Convert to data.frame to avoid sf conflict
favela_area_district_df <- favela_area_district |> st_drop_geometry()

# Now perform the join and compute the percentage
model_data <- model_data |> 
  left_join(favela_area_district_df, by = "ds_nome") |> 
  mutate(
    total_favela_area = replace_na(total_favela_area, units::set_units(0, m^2)),
    pct_favela = as.numeric(total_favela_area / area_total) * 100
  )
```
...
ols_model <- lm(
  deaths_log ~ pct_favela + pct_branca + pct_preta + pct_masc + avg_age + pct_pm,
  data = model_data
)
summary(ols_model)
